...
- `lib/loan-engine.ts` — `generateAmortizationSchedule` calculates `monthlyFee` once at the start and emits `monthlyFee` + `totalPayment` on every row. `calculateRemainingBalance`, `getProjectedPayoffDate`, `getDaysOverdue`, status detection.
  ...
  - **`newTermMonths` semantics**: plazo restante desde el mes del extra (duración de la fase 2, NO mes absoluto). `endMonth = extraMonth + ex.newTermMonths`. A subsequent `REDUCE_PAYMENT` truncates the previous phase to its `extraMonth`. Form labels read "Nuevo plazo restante" (not "total").
  - **Effective cuota helpers**: `getCurrentEffectiveLoanPayment(loan, payments?, extras?)` in `lib/loan-engine.ts` returns the post-recalc French cuota for the next month. `getCurrentEffectiveMonthlyPayment(loan)` in `lib/loan-fees.ts` adds `fees/12` on top. These are the source of truth for the "Cuota mensual" KPI in `CreditSummary` and `CreditDetailHeader`. `CreditCard` (list view) still uses the legacy `getEffectiveMonthlyPayment` (banco cuota + fees) because the list query only loads `_count` of payments/extras.
