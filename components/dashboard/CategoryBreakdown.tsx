...
export function CategoryBreakdown({ items }: CategoryBreakdownProps) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-card via-card to-muted/20 shadow-xl ring-1 ring-black/5"
    >
      ...
      <div className="relative p-6 space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground font-medium">
                Equivalente mensual · ordenado por gasto
              </p>
            </div>
          </div>
          <Button
        ...
        {totalSpent > 0 && totalLimit > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="rounded-xl border border-border/60 bg-gradient-to-br from-blue-50 to-indigo-50 p-3">
              <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-blue-700">
                <BarChart3 className="h-3 w-3" />
                Total categorías
              </div>
              <div className="text-2xl font-extrabold tabular-nums text-blue-900 mt-1">
                {sorted.length}
              </div>
            </div>
            <div
                ...
                >
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span
                        className={`text-[10px] font-bold tabular-nums px-2 py-0.5 rounded-full border ${status.pill}`}
                      >
                        {shareOfTotal.toFixed(0)}% del total
                      </span>
                      <span
                  ...
                  <div className="flex items-center gap-3">
                    <Progress
                      ...
                      className={`h-2.5 flex-1 ${status.color}`}
                    />
                    <span className="text-[11px] text-muted-foreground font-medium shrink-0 w-32 text-right tabular-nums">
