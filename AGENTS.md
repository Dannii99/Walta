...
- **Módulo "Créditos" (`/credits`):**
  ...
  - `components/credits/CreditSummary.tsx` — 4 KPIs del préstamo. KPI "Cuota mensual" muestra `monthlyPayment + calculateTotalMonthlyFees(fees)` con sub-line `+ $X cargos · N/M cuotas` cuando hay fees. `Saldo actual` (de `calculateRemainingBalance`), `Total pagado` (sum payments) y `Próximo pago` (de `getNextPaymentDate`) intactos.
  ...
  - `components/credits/CreditDetailHeader.tsx` — Header card. Nueva prop opcional `onOpenActions?: () => void` que renderiza botón "Acciones" (Zap icon, h-9 px-3, rounded-full, `aria-label="Abrir acciones rápidas"`) entre área vacía y "Editar".
