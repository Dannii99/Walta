...
- **Módulo "Historial" (`/history`):**
  ...
  - Container pattern: `max-w-360 mx-auto p-4 md:px-6 lg:px-10 pb-18 md:pb-6 pt-6 md:pt-8` + inner `max-w-3xl space-y-6 md:space-y-8`.
...
- **Feature "Reducir Cuota" (Abonos con recálculo de cuota)**:
  ...
  - **Helpers vigentes en `lib/loan-engine.ts` + `lib/loan-fees.ts`**: `getCurrentEffectiveLoanPayment(loan, payments?, extras?)` retorna la cuota francesa del próximo mes post-recalcs (fallback a `loan.monthlyPayment` si no hay schedule row). `getCurrentEffectiveMonthlyPayment(loan)` agrega `fees/12` al resultado. **Diferencia con `getEffectiveMonthlyPayment`**: el viejo retornaba `loan.monthlyPayment + fees/12` (banco cuota, fija). El nuevo retorna la **cuota financiera vigente** post-recalcs + fees. `CreditSummary` y `CreditDetailHeader` usan el nuevo. `CreditCard` (list view) sigue con el viejo porque no tiene `payments`/`extraPayments` cargados. `ai-actions.ts` usa el nuevo helper para DRY.
  - **Semántica de `newTermMonths`**: **plazo restante desde el mes del extra** (duración de la fase 2, NO mes absoluto). `endMonth = extraMonth + ex.newTermMonths`. Cuando se inserta un nuevo `REDUCE_PAYMENT` después, la fase previa se TRUNCA a `extraMonth` del nuevo. Labels en forms: "Nuevo plazo restante" (no "total").
